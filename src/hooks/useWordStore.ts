...
interface WordStore {
  ...
  togglePractice: (id: string, needsPractice: boolean) => void;
  clearPracticeLog: () => void;
    ...
    (set, get) => ({
      ...
      addWord: (word, folder) => {
        ...
        set((state) => {
          ...
          const nextFolderNames = state.folderNames.some(f => f.toLowerCase() === folderName.toLowerCase())
            ? state.folderNames : [...state.folderNames, folderName];
          return { words: nextWords, folderNames: nextFolderNames, folders: computeFolders(nextWords, nextFolderNames) };
        });
      },

      togglePractice: (id, needsPractice) => {
        set((state) => ({ words: state.words.map((w) => w.id === id ? { ...w, needsPractice } : w) }));
      },

      clearPracticeLog: () => {
        set((state) => ({ words: state.words.map(w => ({ ...w, needsPractice: false })) }));
      },
      ...
      addFolder: (name) => {
        ...
        set((state) => {
          ...
          return { folderNames: nextFolderNames, folders: computeFolders(state.words, nextFolderNames) };
        });
      },
      ...
      deleteWord: (id) => {
        set((state) => {
          ...
          return { words: nextWords, folders: computeFolders(nextWords, state.folderNames) };
        });
      },
      ...
      deleteFolder: (name) => {
        ...
        set((state) => {
          const nextWords = state.words.map(w => w.folder === name ? { ...w, folder: "General" } : w);
      ...
      importWords: (tsvContent) => {
        ...
        lines.forEach((line, i) => {
          ...
          const [word, folder, practice, date, definition, note] = line.split("\t");
          if (word?.trim()) {
      ...
      exportWords: () => {
        const { words } = get();
        const header = "Word\tFolder\tNeeds Practice\tDate\tDefinition\tNote\n";
        const text = words
          .map((w) => `${w.word}\t${w.folder}\t${w.needsPractice ? "Yes" : "No"}\t${new Date(w.createdAt).toLocaleDateString()}\t${w.definition || ""}\t${w.note || ""}`)
          .join("\n");
